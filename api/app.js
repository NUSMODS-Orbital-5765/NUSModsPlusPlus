...
const {
  validifyAllModulePlan,
  AutoAllocateModulePlan,
} = require("./autoAllocate/autoAllocate");
dotenv.config();
...
app.post("/module-plan/get", jsonParser, (request, response) => {
  ...
  prisma.modulePlan
    .findMany({
      where: { owner: { username: request.body.username } },
      orderBy: { id: "asc" },
    })
    .then((planList) => {
      ...
      response.status(200).send({
        message:
          "Getting Module Plan Successfully from username " +
          request.body.username,
        planList,
      });
    })
...
app.post("/admin/get-profile-with-filter", jsonParser, (request, response) => {
  console.log(
    "POST search REQUEST with faculty filter = " +
      request.body.facultyFilter +
      " and acad filter = " +
      request.body.acadFilter +
      "programme = " +
      request.body.programmeFilter
  );
...
app.post("/admin/add-comment", jsonParser, (request, response) => {
  ...
  prisma.comment
    ...
    .then((commentsList) => {
      console.log(
        `Created Comment Admin to modulePLan ${request.body.nanoid} and admin ${request.body.username}`
      );
      response.status(201).send({
        message: "Comment Created Successfully",
        commentsList,
...
app.post("/module-plan/get-comment", jsonParser, (request, response) => {
  ...
  prisma.comment
    ...
    .then((commentsList) => {
      ...
      response.status(200).send({
        message:
          "Getting Comment Successfully from modulePlan " + request.body.nanoid,
        commentsList,
      });
    })
    .catch((error) => {
...
app.post("/admin/approve", jsonParser, (request, response) => {
  ...
  prisma.ModulePlan.update({
    ...
    .then((res) => {
      ...
      response.status(200).send({
        message:
          "Update Approval Action Admin Successfully for modulePlan " +
          request.body.nanoid,
        res,
      });
    })
